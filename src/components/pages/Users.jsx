...
const getUsers = (number) => {
  const a = Math.floor(number / usersOnPageNumber);
  let users = [];
  for (let i = 0; i < a; i++) { users = users.concat(defaultUsers); }
  if (users.length !== number) {
    users = users.concat(defaultUsers.slice(0, number % usersOnPageNumber));
  }
  return users;
};
