  ...
  return (
    <div>
      ...
      <div className={`rbc-event-expand
         ${event.isDataOpen ? 'open' : 'hidden'}
         ${event.end.getDay() > 3 ? 'left' : 'right'}`}
      >
        ...
        <div className="rbc-event-part">
          ...
          <Link
            to={`/calendar/${event.id.toString()}`}
            className="rbc-event-edit"
            href="/"
          >
            Edit event
          </Link>
...
CustomEvent.propTypes = {
  event: PropTypes.objectOf(PropTypes.oneOfType(PropTypes.string, PropTypes.number))
};
...
class Calendar extends Component {
  ...
  onEventAdd(slotInfo) {
    if (slotInfo.action === 'select') {
      const title = prompt(
        'Enter the event title (press cancel to undo adding)',
